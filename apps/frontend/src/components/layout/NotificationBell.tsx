...
const TYPE_ICONS: Record<string, React.ReactNode> = {
  ...
  DATA_ROOM_UPDATED: <Settings className="h-4 w-4 text-slate-500" />,
  PERMISSION_CHANGED: <Settings className="h-4 w-4 text-slate-500" />,
};
...
function NotificationItem({ notification, onRead }: { notification: Notification; onRead: (id: string) => void }) {
  return (
    <button
      ...
      className={`flex w-full items-start gap-3 px-4 py-3 text-left transition-colors hover:bg-slate-50 ${!notification.isRead ? 'bg-blue-50/50' : ''}`}
    >
      <div className="mt-0.5 flex-shrink-0">
        {TYPE_ICONS[notification.type] ?? <Bell className="h-4 w-4 text-slate-400" />}
      </div>
...
export function NotificationBell() {
  ...
  return (
    <div ref={panelRef} className="relative">
      ...
      {isOpen && (
        <div className="absolute right-0 top-full mt-2 w-80 animate-scale-in rounded-lg border border-slate-200 bg-white shadow-popover z-50">
