...
function TotpInput({ value, onChange }: { value: string; onChange: (v: string) => void }) {
  return (
    <input
      ...
      placeholder="000000"
      className="w-36 rounded-md border border-app-border2 px-3 py-2 text-center font-mono text-lg tracking-widest focus:border-slate-500 focus:outline-none focus:ring-1 focus:ring-slate-500"
    />
  );
}
...
export function MfaSettingsPanel({ isMfaEnabled, onStatusChange }: Props) {
  ...
  if (step === 'done') {
    return (
      <div className="rounded-lg border border-green-200 bg-green-500/10 p-5">
  ...
  if (step === 'setup' && setupResult) {
    return (
      <div className="space-y-5">
        ...
        <div className="flex flex-col items-center gap-4 rounded-lg border border-app-border bg-app-s2 p-5">
          <div className="rounded-lg bg-app-s1 p-2 shadow-sm">
            {/* eslint-disable-next-line @next/next/no-img-element */}
            <img src={setupResult.qrCodeDataUrl} alt="MFA QR code" width={180} height={180} />
          </div>
          <div className="text-center">
            <p className="text-xs text-app-t3 mb-1">Or enter this secret manually:</p>
            <div className="flex items-center gap-2">
              <code className="rounded bg-app-s2 px-2 py-1 text-xs font-mono text-app-text break-all">
                {setupResult.secret}
              </code>
              <button onClick={copySecret} className="text-app-t3 hover:text-app-t2">
        ...
        <div className="space-y-3">
          <p className="text-sm font-medium text-app-t2">Enter the code from your app:</p>
